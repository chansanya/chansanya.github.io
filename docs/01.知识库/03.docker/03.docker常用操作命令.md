...
```shell
### 创建临时容器
docker create --name tingly_tmp ghcr.io/tingly-dev/tingly-box:latest
### 得到配置
docker cp tingly_tmp:/home/tingly/.tingly-box/config.json ./data/config.json
### 删除临时容器
docker rm tingly_tmp

### 查看用户ID
docker-compose up -d && docker-compose exec tingly /bin/bash -c 'id tingly'
### 修正权限
sudo chown -R 999:999 ./data/config.json
```
